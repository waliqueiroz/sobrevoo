...
- **Trajeto muito curto ou quase parado**: se a extensão do trajeto (distância percorrida) ficar abaixo de um mínimo documentado, a ferramenta recusa com erro próprio e mensagem que informa a extensão encontrada e o mínimo exigido, sem produzir plano; acima desse mínimo, o plano é gerado normalmente.
- **Trajeto muito longo** (por exemplo, centenas de quilômetros ou muitas horas): o plano continua suave e proporcional, e a distância da câmera se ajusta para não perder o marcador de vista.
...
- **FR-014**: O mesmo trajeto com os mesmos parâmetros DEVE produzir sempre exatamente o mesmo plano, quadro a quadro e valor a valor, independentemente de máquina, momento ou ordem de execução.
...
- **FR-016**: O sistema DEVE recusar duração ou taxa de quadros que sejam zero, negativas ou não numéricas, com mensagem que aponte o parâmetro inválido; a taxa de quadros DEVE estar entre 1 e 120 quadros por segundo (inclusive), e valores fora desse intervalo DEVEM ser recusados com mensagem que informe o intervalo válido.
...
- **FR-023**: Todos os erros de negócio (parâmetro inválido, duração insuficiente, trajeto curto demais, destino de exportação inválido ou já existente) DEVEM ser distinguíveis entre si e comunicados com mensagem clara e código de saída próprio, sem confundir com erros de leitura do trajeto.
...
- Trajetos extremamente longos (acima de milhares de quilômetros) e planos com milhões de quadros estão fora do uso esperado; a ferramenta pode recusar entradas fora de limites razoáveis, desde que com mensagem clara.
